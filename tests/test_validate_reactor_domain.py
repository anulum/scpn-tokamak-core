...
# SCPN Tokamak Core — reactor-domain validator tests
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"evidence_maturity": "architecture_only"}, "must be [] at architecture_only"),
        ({"claims": ["fast"]}, "claims:"),
        ...
        (
            {"configurations": ["conventional_tokamak", "conventional_tokamak"]},
            "must be unique",
        ),
        (
...
def test_map_cross_check_rejects_assignment_and_pin_drift(tmp_path: Path) -> None:
    ...
    map_path.write_text(
        json.dumps(
            {
                "planned_repositories": ["SCPN-TOKAMAK-CORE"],
                "existing_repositories": [],
                "configuration_assignments": {
