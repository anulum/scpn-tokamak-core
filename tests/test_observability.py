...
# SCPN Tokamak Core — diagnostic and clock semantics tests
...
DERIVED_BINDINGS = {
    ...
    "mode_identity": "declared toroidal and poloidal mode labels",
...
EVENT_BINDINGS = {
    ...
    "event_reference": "synthetic transient onset marker",
    "provenance": "synthetic fixture",
...
def clock_shot() -> ClockModel:
    """Build the synthetic shot-trigger epoch clock."""
    return ClockModel(
        identifier="clk_shot",
...
def channel_elm_train() -> DiagnosticChannelPlan:
    """Build the synthetic recurrent-transient channel."""
...
def channel_equilibrium() -> DiagnosticChannelPlan:
    """Build the synthetic equilibrium-profile channel."""
    return DiagnosticChannelPlan(
...
def test_channel_rejects_unknown_candidate() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="not applicable"):
        _mirnov(candidate_id="open.drive_reference")
...
def test_channel_rejects_inadmissible_carrier() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="not admissible"):
        _mirnov(carrier=SemanticCarrier.EVENT_CYCLE)
...
def test_parser_accepts_integer_numbers() -> None:
    ...
    record = synthetic_plan().to_record()
    record["channels"][1]["sample_rate_hz"] = 1000
    assert plan_from_record(record).channels[1].sample_rate_hz == 1000.0
...
def test_bytes_parser_rejects_nan_literal() -> None:
    ...
    record = synthetic_plan().to_record()
    text = json.dumps(record).replace("1e-05", "NaN")
