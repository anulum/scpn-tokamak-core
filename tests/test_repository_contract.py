...
# SCPN Tokamak Core — repository-level truth tests
...
REQUIRED_PATHS = (
    ...
    "src/scpn_tokamak_core/device/__init__.py",
    "src/scpn_tokamak_core/device/envelope.py",
    "src/scpn_tokamak_core/device/model.py",
    "src/scpn_tokamak_core/device/cad.py",
    "conftest.py",
    "reactor-domain.json",
...
def test_manifest_declares_exact_tokamak_assignment() -> None:
    """The manifest owns exactly the two mapped tokamak configurations."""
    manifest = load_json_object(REPO / "reactor-domain.json")
    assert manifest["project"] == "SCPN-TOKAMAK-CORE"
...
def test_package_agrees_with_manifest_truth() -> None:
    ...
    from scpn_tokamak_core import OWNED_CONFIGURATIONS, RegistryBinding
...
def test_typed_package_marker_exists() -> None:
    ...
    assert (REPO / "src" / "scpn_tokamak_core" / "py.typed").is_file()
