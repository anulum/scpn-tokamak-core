...
# SCPN Tokamak Core — diagnostic and clock semantics
...
identities for the tokamak configurations this repository owns, aligned
...
from scpn_tokamak_core.configuration import ConsistencyFinding
...
MHD_BAND_HZ: Final = (1.0e3, 1.0e5)
TRANSIENT_TIMING_CEILING_S: Final = 1.0e-4

SHOT_DURATION_CEILING_S: Final = 1.0e02
...
class CandidateProfile:
    ...
    def __post_init__(self) -> None:
        ...
        for configuration in self.configurations:
            if configuration not in ("conventional_tokamak", "spherical_tokamak"):
                raise DiagnosticPlanError(
                    "candidate.configurations: "
                    f"{configuration!r} is not owned by SCPN-TOKAMAK-CORE"
                )
...
APPLICABLE_CANDIDATES: Final = (
    ...
    CandidateProfile(
        ...
        phenomenon="recurrent edge or core transient sequence",
        configurations=("conventional_tokamak", "spherical_tokamak"),
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-TOKAMAK-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def __post_init__(self) -> None:
        ...
        self._validate_transformations(frames_by_id)
        self._validate_topology(clocks_by_id)
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            " Hz is outside the typical tokamak tearing/kink "
                            "range 1-100 kHz (Hutchinson 2002, ch. 10)"
                        ),
                    )
                )
            if (
                observability_class is ObservabilityClass.EVENT_RELATIVE
                and channel.timing_uncertainty_s is not None
                and channel.timing_uncertainty_s > TRANSIENT_TIMING_CEILING_S
            ):
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            " is coarser than the 10-100 us rise scale of "
                            "ELM and sawtooth transients (Zohm 1996)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the tokamak flat-top scale "
                            "of up to ~100 s "
