...
bind. Four candidates apply to the configurations this repository owns:
`closed.equilibrium_profiles` (noncyclic feature),
`closed.recurrent_transient` (event relative),
...
1. The capability `diagnostic_clock_semantics` is implemented as a
   declaration model (`src/scpn_tokamak_core/observability.py`): frozen
...
3. Alignment is enforced structurally: a channel must address an
   ...
   (event-relative to the shot epoch, numerical-only to the simulation
   clock, cyclic to the facility monotonic clock).
4. Timing is fail-closed where physics fixes the bound: the Nyquist
   criterion for cyclic channels and clock resolution at or below the
   declared event-timing uncertainty. Device-typical band checks are
   advisory only: the tokamak tearing/kink band 1–100 kHz
   (I. H. Hutchinson, Principles of Plasma Diagnostics, 2nd ed., 2002,
   ch. 10) and the 10–100 µs ELM/sawtooth rise scale
   (H. Zohm, PPCF 38 (1996) 105).
5. Every applicable candidate must be planned or explicitly deferred
   with a reason; every channel is `synthetic=True` by hard invariant.
   No SPO semantic-profile ingress is declared: the profile registry's
   `ingress_state` for these configurations stays `not_declared` until
   a real, versioned adapter exists.

## Consequences

- The manifest gains the capability
  `diagnostic_clock_semantics` at `computational_prototype`
  (evidence: `VALIDATION.md#diagnostic-and-clock-semantics`); the
  repository-level maturity remains `computational_prototype` under the
  ADR 0002 ceiling rule.
